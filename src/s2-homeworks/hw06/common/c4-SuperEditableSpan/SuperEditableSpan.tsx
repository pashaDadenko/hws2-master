...
const SuperEditableSpan: React.FC<SuperEditableSpanType> = ({ autoFocus, onBlur, onEnter, spanProps, ...restProps }) => {
	...
	const [value, setValue] = useState<string>(() => restoreState<string>('editable-span-value', (restProps.value as string) || ''));
	...
	const onChangeTextCallback = (newValue: string) => {
		setValue(newValue);
		restProps.onChange?.({ target: { value: newValue } } as React.ChangeEvent<HTMLInputElement>);
	};
		...
		<>
			{editMode ? (
				<SuperInputText
					...
					className={s.input}
					value={value}
					onChange={(e) => onChangeTextCallback(e.currentTarget.value)}
